...
public class GPUPBD : MonoBehaviour
{
    ...
    [Tooltip("렌더링에 사용할 Material. RenderObj Shader를 사용해야 합니다.")]
    ...
    private void setupShader()
    {
        ...
        {
            runtimeMaterial.SetBuffer(Shader.PropertyToID("vertsBuff"), vertsBuff);
            runtimeMaterial.SetBuffer(Shader.PropertyToID("triBuff"), triBuffer);
        }
    }
    private void setBuffData()
    {
        ...
        if (runtimeMaterial != null)
        {
            runtimeMaterial.SetMatrix("TRSMatrix", trs);
            runtimeMaterial.SetMatrix("invTRSMatrix", trs.inverse);
        }
    }
    ...
    void setup()
    {
        if (material == null)
        {
            Debug.LogError("Material이 할당되지 않았습니다. Inspector에서 Material을 할당해주세요.");
            return;
        }
        
        // 원본 Material을 보호하기 위해 런타임 인스턴스 생성
        runtimeMaterial = new Material(material);
        
        // Shader가 RenderObj인지 확인
        if (runtimeMaterial.shader.name != "RenderObj")
        {
            Shader renderObjShader = Shader.Find("RenderObj");
            if (renderObjShader != null)
            {
                runtimeMaterial.shader = renderObjShader;
                Debug.Log("Material의 Shader를 RenderObj로 변경했습니다.");
            }
            else
            {
                Debug.LogError("RenderObj Shader를 찾을 수 없습니다.");
                return;
            }
        }
        ...
        // _BaseColor가 검정색이면 흰색으로 설정 (Meta Quest 대비)
        if (runtimeMaterial.HasProperty("_BaseColor"))
        {
            Color baseColor = runtimeMaterial.GetColor("_BaseColor");
            if (baseColor.r + baseColor.g + baseColor.b < 0.01f)
            {
                runtimeMaterial.SetColor("_BaseColor", Color.white);
                Debug.Log("_BaseColor가 검정색이어서 흰색으로 설정했습니다. Inspector에서 Material 색상을 변경하세요.");
            }
        }
        
        // Smoothness와 Metallic 기본값 설정
        if (runtimeMaterial.HasProperty("_Smoothness") && runtimeMaterial.GetFloat("_Smoothness") == 0.0f)
        {
            runtimeMaterial.SetFloat("_Smoothness", 0.5f);
        }
        if (runtimeMaterial.HasProperty("_Metallic") && runtimeMaterial.GetFloat("_Metallic") == 0.0f)
        {
            runtimeMaterial.SetFloat("_Metallic", 0.0f);
        }
        
        Debug.Log($"Material 설정 완료 - Color: {runtimeMaterial.GetColor("_BaseColor")}, Smoothness: {runtimeMaterial.GetFloat("_Smoothness")}");
    ...
    void renderObject()
    {
        ...
        Graphics.DrawProcedural(runtimeMaterial, bounds, MeshTopology.Triangles, triArray.Length,
